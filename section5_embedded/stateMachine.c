...
/**
 * @brief Filtrira mehanički šum (debounce) s digitalnog pina.
 * @param raw_pin_state Trenutno stanje pina (0 = pritisnuto, 1 = otpušteno).
 * @return Službeno, stabilizirano stanje gumba.
 */
ButtonState_t debounce_button(uint8_t raw_pin_state) {
    ...
    if ((raw_pin_state == 0 && stabilno_stanje == BUTTON_RELEASED) ||
        ...
        // Ako ostane 3 zaredom prihvaća se promjena
