...
begin
    ...
    begin
        if rst = '1' then
            ...
            -- Resetiramo defaultne izlaze na pocetku svakog takta
            error <= "0000";
            change <= 0;

            case current_state is
                when IDLE =>
                    -- elsif da sprijecimo vise stvari od jednom istovremeno
                    if coins /= prev_coins and coins /= "0000" then
                        ...
                        if coin_value = -1 then
                            error <= "0001"; -- Error: Nevazeca kovanica
                    ...
                    --Povrat kusura
                        ...
                        balance <= 0;      -- Instanto resetiraj balance na nulu kako trazi PDF
                    ...
                    --Obrada narudzbe
                        ...
                        if prod_idx < 1 or prod_idx > 20 then
                            ...
                            -- Racunanje cijene gdje je prvih 5 1.25 EUR, a ostali +37 centi
                            if prod_idx <= 5 then
                                prod_price := 125; 
                            else
                            ...
                            if balance >= prod_price then
                                ...
                                -- Zbog FSMa jedan takt ide vise radi prijelaza
                ...
                when DISPATCH =>
                    ...
                    --Ilegalne radnje u isporuci, ignoriramo i dizemo zastavice
                    if coins /= prev_coins and coins /= "0000" then
                        error <= "0100"; -- Ubacivanje kovanice tijekom isporuke
                    end if;
                    if product /= prev_product and product /= "00000" then
                        error <= "0101"; --Trazenje proizvoda tijekom isporuke
                    end if;
                    if change_request = '1' and prev_change_request = '0' then
                        error <= "0110"; --Trazenje povrata tijekom isporuke
                    end if;

            end case;

            -- Spremanje trenutnog stanja za edge detection u iducem ciklusu
            prev_coins <= coins;
